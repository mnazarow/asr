...
| Самые медленные задания | `slowest` | что именно тормозило и на какой стадии |
...
Кроме среднего, считаются перцентили: **p50** (медиана), **p90**, **p95**, **p99**. Разрыв между медианой и p95 говорит больше, чем среднее: если p50 = 0,12, а p95 = 0,9, значит одна запись из двадцати обрабатывается в семь раз медленнее обычного — стоит посмотреть, что это за записи, в разделе «Самые медленные задания».
...
Средняя уверенность за месяц — число, которое ничего не сообщает, а средняя по всем моделям может не сдвинуться, когда одна модель стала хуже, а другую стали реже использовать. Карточка **«Дрейф уверенности»** сравнивает *распределение* средней уверенности по заданиям за период с четырьмя неделями до него — по каждой модели и по всем вместе — двухвыборочным критерием Колмогорова — Смирнова: половина записей стала лучше, половина хуже — среднее то же, а критерий это видит. Пороги — по Deepgram: сдвиг среднего в плохую сторону на 10 % и p < 0,05 вместе — критично, что-то одно — предупреждение; сдвиг вверх вердикта не даёт. Критерий считает расхождение в точках, а не после каждого шага слияния: на выборках с повторяющимися значениями (а они обычны — запись из кеша копирует уверенность оригинала) прежний счёт раздувал расхождение на длину блока повторов и выносил «критично» двум одинаковым выборкам. Вердикт выносится, когда в обеих выборках от тридцати заданий, — иначе «мало данных», а не ложная тревога. В таблице — медианы, относительный сдвиг, p и доля заданий с уверенностью ниже 0,75 за период и в базе. Граница периода у дрейфа и контрольных карт та же, что у остальных разделов отчёта, а база отсчитывается от неё назад; у периода «всё время» границы нет, а значит, нет и «до» — дрейф показывает «мало данных», а карты берут последние тридцать дней.

**«Контрольные карты распознавания»** — уверенность, доля заданий с низкой уверенностью и доля подозрительных сегментов по дням с пределами μ ± 2σ и μ ± 3σ, посчитанными по двадцати восьми дням до периода. Точки за 3σ — критично, за 2σ — предупреждение, семь дней подряд по одну сторону от среднего — сдвиг, который ни один день по отдельности не выдаёт. Дни без заданий серию прерывают. За период короче суток карта рисуется, но вердиктов не выносит: пределы посчитаны по суточным точкам, и один час на восьми разговорах уходит за 3σ обычным биномиальным шумом — «критично» при каждом открытии раздела за час.
...
### Очередь ручной проверки и согласие моделей

![Очередь ручной проверки и согласие моделей](images/67-analytics-review-agreement.png)

Эталоны не появляются сами: нужен человек и порядок, по которому ему дают записи. **Очередь ручной проверки** пополняется раз в сутки по правилу Deepgram — доля случайных записей (`review_daily_share`, по умолчанию 1 %) плюс несколько самых неуверенных из нижней четверти по уверенности (`review_daily_low`, по умолчанию три), не больше `review_daily_max` за день. Случайные — потому что только по случайной выборке WER честный: выборка из одних трудных записей завышает его, из лёгких — занижает. Неуверенные — потому что там ошибки чаще, и исправлений на час прослушивания выходит больше. Записи из кеша, контрольные прогоны и записи, у которых эталон уже есть, в очередь не попадают. Дневной предел делится между двумя причинами, а не достаётся той, что набралась первой: на потоке в две тысячи записей случайные занимали его целиком, и неуверенные — вторая половина замысла — не попадали в очередь никогда. На выборке меньше четырёх записей «нижней четверти» не бывает, и выдумывать её не нужно.

Кнопка «Открыть» ведёт в карточку записи сразу на вкладку «Эталон»; сохранённый эталон закрывает строку сам, с именем того, кто проверял. «Пропустить» — для записей, которые слушать незачем. Администратор может пополнить очередь, не дожидаясь суточного захода; первый отбор после запуска сервера случается через сутки. Очередь есть и в API (`GET /api/review`, `PUT /api/review/{id}`), в сводке по расписанию и в метрике `asrhub_review_pending`.

**Согласие моделей** — контроль без эталона. Если задана контрольная модель (`control_model` — модель другого семейства: GigaAM против Whisper, а не две версии одного), сервер раз в сутки заново распознаёт несколько случайных записей (`control_daily_jobs`, по умолчанию пять) и считает расхождение двух расшифровок — WER контрольной относительно исходной. Какая из двух права, без эталона неизвестно, и абсолютный уровень ничего не значит: у пары GigaAM — Whisper обычные 10–20 %. Значит ход: рост среднего расхождения на четверть к прошлому такому же периоду — предупреждение, на половину — критично; на выборке меньше пяти прогонов вердикта нет. Карточка показывает ряд по дням, пары моделей и записи, на которых модели разошлись сильнее всего, — с пометкой «шум», если запись шумная: расхождение на плохом звуке ожидаемо.

Контрольные прогоны — обычные задания с низким приоритетом (считаются, когда очередь свободна), источником `control` и меткой «контроль»; в разбор содержания они не попадают, иначе каждый проверенный разговор считался бы дважды. Исходная запись получает событие «Контрольная модель …: расхождение N %». Для мониторинга — `asrhub_model_disagreement` по основным моделям; для API — `GET /api/control` и `POST /api/control/run`.
...
```
asrhub_jobs_current{status="queued"} 6
asrhub_jobs_current{status="running"} 2
asrhub_jobs_current{status="completed"} 1284
asrhub_jobs_current{status="failed"} 37
asrhub_audio_seconds_day 68606.9
asrhub_words_day 156196
...
Метрики за сутки (`audio_seconds_day`, `words_day`, перцентили) плюс разрез по моделям и последний замер загрузки железа. Точка `/api/metrics` доступна без ключа — её обычно закрывают на уровне прокси, а не приложения.

Имена здесь намеренно другие, чем у подсистемы мониторинга. Это запасной путь — он отвечает, когда подсистема не поднялась, — и величины у него другого рода: `asrhub_jobs_current` это «сколько заданий в статусе сейчас», а не счётчик с момента запуска. Пока имена совпадали, Prometheus видел под одним рядом то накопительный счётчик, то убывающую величину, и `rate()` считал каждое снижение сбросом счётчика, завышая поток в разы.
...
groups:
  - name: asrhub
    rules:
      - alert: ОчередьРастёт
        expr: asrhub_jobs_current{status="queued"} > 50
        for: 15m
      - alert: МалоМестаНаДиске
        expr: asrhub_disk_free_gb < 20
        for: 5m
      - alert: МногоОшибок
        expr: increase(asrhub_jobs_current{status="failed"}[30m]) > 20
        for: 10m
```
