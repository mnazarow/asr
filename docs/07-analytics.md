...
Каждый доступен отдельно: `GET /api/analytics/<раздел>?period=week`. Разделы, добавленные позже (`weekly`, `cache`, `reliability`, `audio`, `resources`, `quality`, `tags`, `queue`), устроены так же.
