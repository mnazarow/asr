...
Отдельно стоит движок `postprocess`. Он ставится ради расстановки знаков препинания, а собирать из исходников нужно было только нормализатор чисел NeMo (`nemo-text-processing` → `pynini`). Теперь эта часть вынесена в `requirements/engines/optional/postprocess.txt`: она ставится вместе с движком, но если не собирается — движок всё равно устанавливается, а в журнале появляется предупреждение. Нормализация чисел при этом не пропадает: сервер берёт `rus2num`, а если нет и его — встроенную русскую нормализацию (целые числа до миллиардов и проценты). Поставить NeMo позже, когда компилятор появится:
