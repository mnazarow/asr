...
- **`nemo-toolkit-asr` требует `protobuf~=5.29.5`, а что-то другое — 6+.** Это не наша поломка и починить её нельзя: `opentelemetry` (приезжает вместе с `pyannote.audio` для диаризации) требует `protobuf>=6.33.5`, а NeMo прибит к 5.29. Два требования несовместимы в принципе. Работает обычно то, что установилось последним; если нужен именно NeMo, поставьте `protobuf~=5.29.5` вручную и не обновляйте движки, тянущие 6+.
- **Прочие расхождения.** Полный список всегда доступен: `/opt/asrhub/venv/bin/pip check`.

Часть расхождений создана нарочно и в списке не показывается. Движки, которые ставятся с `--no-deps` (`gigaam` — его пины `onnx==1.19.*` и `onnxruntime==1.23.*` невыполнимы под свежий Python; `nemo-text-processing` — прибит к `pynini` без готового колеса), по определению жалуются на версии своих зависимостей: это следствие нашего решения, а не находка. Список таких пакетов собирается из самих файлов-спутников `no-deps/`, поэтому не расходится с ними. Проверка, которая ругается на сделанное нарочно, приучает пролистывать её целиком — и тогда настоящая находка тонет вместе с остальными.
