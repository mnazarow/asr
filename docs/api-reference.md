...
Всего маршрутов: **74**, операций: **83**. Справочник собран из схемы OpenAPI работающего сервера, а примеры ответов сняты с него же, поэтому расходиться с действительностью им негде.
...
| `POST` | `/api/auth/login` | Вход по логину и паролю | ключ с правом записи (**admin** или **user**) |
| `POST` | `/api/auth/logout` | Выход | ключ с правом записи (**admin** или **user**) |
| `GET` | `/api/auth/me` | Кто я | любой действующий ключ |
| `POST` | `/api/auth/password` | Смена своего пароля | ключ с правом записи (**admin** или **user**) |
...
| `GET` | `/api/users` | Список учётных записей | любой действующий ключ |
| `POST` | `/api/users` | Завести учётную запись | ключ с правом записи (**admin** или **user**) |
| `DELETE` | `/api/users/{user_id}` | Удалить учётную запись | ключ с правом записи (**admin** или **user**) |
| `PATCH` | `/api/users/{user_id}` | Изменить учётную запись | ключ с правом записи (**admin** или **user**) |
...
```json
{
  "overview": {
    "period": "week",
    "generated_at": 1788434494.9597058,
    "jobs": {
      "total": 266,
      "completed": 249,
      "failed": 9,
    ...
    "volume": {
      "audio_seconds": 323290.5,
      "audio_hours": 89.8,
      "processing_seconds": 43436.6,
      "words": 736819,
      "characters": 4420914,
      "segments": 34765,
      "files_per_hour": 1.48,
...
```json
{
  ...
  "hardware": {
    ...
    "warnings": [
      ...
      "На диске свободно 16.1 ГБ. Полный набор моделей занимает свыше 100 ГБ."
    ]
  },
  "recommended": {
    "device": "cpu",
