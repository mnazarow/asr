...
SECTIONS: list[tuple[str, str, tuple[str, ...]]] = [
    ...
    ("Языковая модель",
     ...
     ("/api/llm",)),
    ("Тренды",
     "Как менялось со временем всё, что сервер измеряет: шестьдесят два "
     "показателя в девяти группах, ряды на общих корзинах времени, "
     "сравнение с прошлым периодом, разложение по часам недели, пары "
     "показателей с наибольшей связью и выгрузка таблицей. Разрез по "
     "владельцу тот же, что у аналитики; показатели железа — только "
     "администратору.",
     ("/api/trends",)),
    ("Телефония",
     "Забор записей разговоров с АТС Asterisk: состояние импорта, проверка "
     "связи с источником, заход по требованию и журнал звонков с отбором по "
     "направлению, очереди, оператору и номеру. Настройки станции и ручной "
     "заход — только администратору.",
     ("/api/telephony",)),
    ...
    ("Контроль качества работы операторов",
     "Проверка человеком поверх балла автомата: очередь с назначением и "
     "сроком, исход проверки и калибровка проверяющих. Главное число здесь "
     "не средний балл, а сдвиг: насколько проверяющий систематически строже "
     "или мягче автомата. Набрать порцию проверок может только "
     "администратор.",
     ("/api/qa",)),
    ("Журнал доступа",
     "Кто, когда и что сделал: второй журнал, устроенный от человека, а не "
     "от запроса. Ведётся отдельно от журнала сервера, живёт своим сроком и "
     "доступен только администратору — открывать его тому, о ком он "
     "ведётся, значит подсказывать, что именно записано.",
     ("/api/audit",)),
    ("Обратная запись в CRM",
     "Проверка того, что уйдёт в Bitrix24 или amoCRM: адрес, заголовки и "
     "тело запроса. По умолчанию ничего не отправляется — комментарий, "
     "попавший не в ту карточку, из ленты уже не убрать.",
     ("/api/crm",)),
    ("Обслуживание",
...
SCOPED_ROUTES = {"/api/jobs", "/api/analytics", "/api/analytics/{section}",
                 ...
                 "/api/trends", "/api/telephony/calls", "/api/telephony/status",
                 "/api/telephony/dimensions"}
...
ADMIN_ROUTES = {
    ...
    # Проверка связи с АТС и заход по требованию — работа со станцией
    # целиком, и ответ проверки несёт адрес источника: только администратор.
    ("/api/telephony/test", "post"), ("/api/telephony/scan", "post"),
}
