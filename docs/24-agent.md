...
1. **По шаблону** `filename`, если он задан.
2. **По идентификатору звонка**: файл, в имени которого встречается `uniqueid`. Так работает `MixMonitor` с `${UNIQUEID}` в имени — самый надёжный случай.
...
1. `POST /api/telephony/agent/hello` — «я такой-то, вот станция, версия и моё состояние». В ответ: идентификатор агента, предел размера записи, правила отсева (`min_duration_s`, `skip_unanswered`), признак «включён» и задание, если его поставили в разделе «АТС».
...
3. `POST /api/telephony/agent/call` — звонок и его запись одним многочастным запросом: поле `meta` (JSON со сведениями о звонке), поле `file` (сама запись, необязательное). Звонок без записи тоже приезжает — он нужен в журнале и в счётчиках, просто распознавать нечего.
