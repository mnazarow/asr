...
Часть весов скачивается только с ним: `pyannote` для диаризации требует его всегда, отдельные сборки GigaAM и Whisper — тоже. Задаётся при установке:
...
Токен попадает в `env.sh` каталога данных с правами `0640` — оттуда его читает служба через `EnvironmentFile`. Установщик берёт его и из окружения (`HF_TOKEN` или `HUGGING_FACE_HUB_TOKEN`), если ключ не задан.

Для уже установленного сервера достаточно дописать строку и перезапустить службу:

```bash
echo 'HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx' | sudo tee -a /var/lib/asrhub/env.sh
sudo chmod 640 /var/lib/asrhub/env.sh
