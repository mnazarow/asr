...
```
ASR Hub 3.1.2 — запуск контейнера
...
Разница между «перезагрузка поможет» и «не поможет» считается, а не угадывается: скрипт сравнивает версию модуля в памяти (`/proc/driver/nvidia/version`) с той, что лежит на диске (`modinfo -F version nvidia`). Совпали — модуль не пересобрался, и reboot потратит простой впустую; тогда скрипт отправляет в `dkms`.
