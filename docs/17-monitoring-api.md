...
```json
{
  "timestamp": 1790246350.8409874,
  "collected_at": "2026-09-24T10:39:10+0000",
  "metrics": [
    {
...
Отправляет текущий снимок немедленно и возвращает результат. Приёмник при этом не сохраняется и в рассылку не встаёт — настройку удобно проверить до того, как записать её в конфигурацию. Сбой любого рода (опечатка в порту, приёмник, отвечающий не по HTTP, перенаправление) возвращается в `error`; учётные данные из адреса в тексте ошибки маскируются.
