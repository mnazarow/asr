...
Управление — текстовые сообщения JSON, звук — двоичные кадры. Ключ предъявляется так же, как на `/ws`: одноразовым билетом в параметре `ticket` либо ключом в заголовке или параметре. Поток — это работа, а не чтение, поэтому ключу с ролью `readonly` он закрыт (код закрытия `4403`), а выключенный параметром `stream_enabled` — всем (`4404`).
