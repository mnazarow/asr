...
**Таблица за период.** Все показатели со значением за период, крайними значениями, последним, изменением и вердиктом.
...
Так же считается и **значение за весь период** в таблице — по суммам сырья за всё окно, а не средним значений корзин. Прежде при шаге в сутки день с одним упавшим заданием и день с девяноста девятью успешными давали «доля успешных 50 %» при настоящих 99 %; изменение к прошлому периоду и вердикт наследовали ту же ошибку. Для счётных показателей («Заданий», «Вопросов», «Сбоев модели») значение за период — среднее на корзину, где пустая корзина считается нулём: в ней и правда ничего не пришло. Считается оно от первой корзины с данными, чтобы окно «год» на сервере, поставленном месяц назад, не делило месячный поток на триста шестьдесят пять дней. Сумма за период («total» в ответе) есть только у счётных показателей: сумма процентов по корзинам — не число.
