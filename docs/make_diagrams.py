...
# ===========================================================================
# Схема 8. Аналитика: что есть и что предлагается
# ===========================================================================

def diagram_analytics_roadmap() -> None:
    c = Canvas(1560, 900, "Аналитика ASR Hub: что есть и что предлагается",
               "Три столбца — три уровня зависимости от моделей; стрелки — что на чём строится")

    # Столбец 1: уже есть
    c.group(30, 100, 460, 760, "УЖЕ ЕСТЬ — БЕЗ МОДЕЛЕЙ")
    есть = [
        ("Показатели разговора", ["тональность и её ход, темп, паузы,",
                                  "перебивания, монологи, паразиты"], BLUE),
        ("Смысл по словарям", ["обещания и сроки, суммы и даты,",
                               "тревожные слова, вопросы, вежливость"], AQUA),
        ("Скрипт разговора", ["редактор пунктов, проверка примет",
                              "на настоящей записи"], GREEN),
        ("Свод и разрезы", ["период к периоду, по операторам,",
                            "часам, моделям; связи; выводы словами"], VIOLET),
        ("Доставка", ["сводка по расписанию, Prometheus,",
                      "тревоги, выгрузка, отборы в результатах"], MAGENTA),
        ("Здоровье распознавания", ["уверенность и WER по времени —",
                                    "среднее, без срезов и дрейфа"], ORANGE),
    ]
    for i, (title, items, colour) in enumerate(есть):
        c.box(50, 140 + i * 118, 420, 100, title, items, accent=colour)

    # Столбец 2: предлагается без моделей
    c.group(530, 100, 500, 760, "ПРЕДЛАГАЕТСЯ — ПРАВИЛА И СЛОВАРИ")
    правила = [
        ("A. Доложить показатели", ["баланс речи, монологи, интерактивность,",
                                    "терпение, наложение в секундах, нормы"], BLUE),
        ("B. Категории обращений", ["И / ИЛИ / НЕ / РЯДОМ, кто сказал, окно;",
                                    "динамика, драйверы негатива, трекеры"], AQUA),
        ("C. Оценка оператора", ["стоп-слова, раскрытия в секундах, мат,",
                                 "веса и балл 0–100, карточка, коучинг"], GREEN),
        ("D. Исход разговора", ["фрустрация, решён / не решён / эскалация,",
                                "повторное обращение, возражения"], VIOLET),
        ("E. Здоровье распознавания", ["галлюцинации по сегментам, дрейф",
                                       "уверенности, WER по срезам, звук на входе"], ORANGE),
        ("F. Персональные данные", ["маскирование в выгрузках,",
                                    "удаление по требованию, согласие"], MAGENTA),
    ]
    for i, (title, items, colour) in enumerate(правила):
        c.box(550, 140 + i * 118, 460, 100, title, items, accent=colour)

    # Стрелки «строится на»
    c.arrow(470, 190, 550, 190, stroke=BLUE, width=1.4)
    c.arrow(470, 426, 550, 308, stroke=GREEN, width=1.4, dash="4 3")
    c.arrow(470, 780, 550, 662, stroke=ORANGE, width=1.4)
    c.arrow(780, 240, 780, 258, stroke=FAINT, width=1.2)
    c.arrow(780, 358, 780, 376, stroke=FAINT, width=1.2)
    c.arrow(780, 476, 780, 494, stroke=FAINT, width=1.2)

    # Столбец 3: с языковой моделью
    c.group(1070, 100, 460, 760, "ПО ЖЕЛАНИЮ — ЛОКАЛЬНАЯ ЯЗЫКОВАЯ МОДЕЛЬ")
    c.box(1090, 140, 420, 176, "G. Смысловой слой",
          ["резюме разговора", "причина обращения и исход — из списка",
           "действия к исполнению", "умные трекеры по смыслу",
           "ответы на вопросы скоркарты с цитатой",
           "", "открытая модель на RTX 5090, всё офлайн"], accent=RED)
    c.box(1090, 350, 420, 120, "Обвязка",
          ["очередь с ограничением, кеш по хешу,",
           "таймауты, заглушка в тестах,",
           "распознавание в приоритете"], accent=DIM)
    c.box(1090, 505, 420, 118, "E11. Контрольная модель",
          ["вторая модель распознаёт выборку,",
           "расхождение гипотез — сигнал",
           "без эталона"], accent=ORANGE)
    c.box(1090, 660, 420, 180, "Что остаётся людям",
          ["ручная проверка выборки — эталон растёт",
           "нормы от своего архива, а не заёмные",
           "решение, что из этого нужно"], accent=FAINT, dash="5 4")
    c.arrow(1010, 308, 1090, 228, stroke=AQUA, width=1.4, dash="4 3")
    c.arrow(1010, 426, 1090, 250, stroke=GREEN, width=1.4, dash="4 3")
    c.arrow(1300, 316, 1300, 350, stroke=RED, width=1.2)
    c.arrow(1010, 662, 1090, 564, stroke=ORANGE, width=1.4)

    c.text(30, 885, "Сплошные стрелки — на чём строится; пунктирные — что становится подсказкой: "
           "скрипт превращается в частный случай категории, а категории и скоркарта "
           "подсказывают языковой модели, из чего выбирать.",
           size=11.5, fill=FAINT)
    c.save("diag-08-analytics-roadmap")
