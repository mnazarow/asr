...
install_systemd() {
  ...
  # Если --user не задан, берём пользователя из УЖЕ СТОЯЩЕГО юнита. Без
  # этого документированная команда смены порта
  # (`bash scripts/service.sh install --port 8081`) молча возвращала службу
  # под root: установщик специально заводит системного пользователя
  # asrhub — «делать это от root незачем», — а переустановка юнита сбивала
  # его обратно. Сервер, который принимает файлы из сети и исполняет код
  # движков, снова работал от root, и никто об этом не узнавал.
  if [[ -z "${run_user}" && "${user_mode}" -eq 0 && -r "${unit_path}" ]]; then
    run_user="$(sed -n 's/^User=//p' "${unit_path}" | head -1)"
    [[ -n "${run_user}" ]] && info "Пользователь службы взят из установленного юнита: ${run_user}"
  fi
  if [[ -z "${run_user}" && "${user_mode}" -eq 0 ]]; then
    run_user="root"
    warn "Пользователь службы не задан — юнит будет работать от root."
    warn "Так делать не стоит: укажите --user ИМЯ (обычно asrhub)."
  fi
