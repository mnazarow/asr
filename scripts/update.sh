...
engine_package() {
  ...
  while IFS= read -r line; do
    ...
    # Ключ pip в начале файла («--extra-index-url …») — не пакет движка.
    [[ "${name}" == -* ]] && continue
...
if [[ "${DOCKER_MODE}" -eq 1 ]]; then
  ...
  # Движки успели перетянуть общие пакеты под свои пины. Возвращаем то, что
  ...
  # тоже до проверки: иначе она снова назовёт их, и пункт выйдет с «!»
  # из-за того, что мы сами же поставили и сами же знаем, как убрать.
...
# Порт спрашиваем у самой службы, а не у файла: `--port` в строке запуска
# юнита перекрывает config.yaml, и они расходятся молча — сервер слушает
# один порт, в конфигурации записан другой, а проверка стучалась в тот, что
# в файле, и объявляла работающий сервер мёртвым. Разбор порядка и очистка
# значения — в server_port_hint.
PORT="$(server_port_hint "${DATA_DIR}" "${PREFIX}" \
        "$( [[ "${DOCKER_MODE}" -eq 1 ]] && echo docker || echo native )")"
