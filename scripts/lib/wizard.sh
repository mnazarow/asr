...
wizard_valid_hf_token() {
  ...
  local token="$1"
  [[ -z "${token}" ]] && return 0
  if [[ ! "${token}" =~ ^hf_[A-Za-z0-9_-]{16,}$ ]]; then
    warn "Токен Hugging Face выглядит так: hf_ и ещё не меньше шестнадцати знаков."
    hint "Взять его: https://huggingface.co/settings/tokens — права «read» достаточно."
    hint "Оставьте поле пустым, если токен не нужен."
    return 1
  fi
  return 0
}
...
wizard_pad() {
  # Выравнивание по ШИРИНЕ В СИМВОЛАХ, а не в байтах.
  #
  # `${#var}` считает символы только в locale с UTF-8. На сервере, куда зашли
  # по ssh без проброса локали, LANG обычно пуст — и «Распознавание на лету»
  # получает длину 40 вместо 21. Это ломало не только выравнивание: строка
  # ниже была `(( length < width )) && printf …`, а `(( ))` возвращает
  # единицу, когда выражение ложно. Последняя команда функции с кодом 1 под
  # errexit обрывала установку прямо на сводке — с сообщением «Сбой на шаге:
  # неизвестный шаг», по которому причину не найти.
  #
  # Продолжающие байты UTF-8 (0x80–0xBF) выкидываем: то, что осталось, по
  # длине равно числу символов. В UTF-8 locale подстановка ничего не меняет,
  # там `${#…}` и так считает символы.
  local text="$1" width="${2:-28}" plain length
  plain="${text//[$'\x80'-$'\xBF']/}"
  length="${#plain}"
  printf '%s' "${text}"
