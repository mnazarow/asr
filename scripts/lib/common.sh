...
# Версия берётся из файла VERSION рядом с деревом скриптов, а не вписывается
# числом: вписанное число расходится с файлом при первом же выпуске, и
# установщик отчитывается об одной версии, а ставит другую. Литерал остаётся
# запасным вариантом — на случай, если скрипт запущен отдельно от дерева.
# Только встроенные средства оболочки: ни dirname, ни cat, ни tr. Этот файл
# подключают и там, где PATH урезан до нескольких команд (так проверяется
# работа без curl), и внешняя команда в самом начале обрывала бы всё.
ASRHUB_VERSION=""
if [[ -r "${BASH_SOURCE[0]%/*}/../../VERSION" ]]; then
  read -r ASRHUB_VERSION < "${BASH_SOURCE[0]%/*}/../../VERSION" || ASRHUB_VERSION=""
fi
ASRHUB_VERSION="${ASRHUB_VERSION//[$'\t\r\n ']/}"
[[ -n "${ASRHUB_VERSION}" ]] || ASRHUB_VERSION="3.1.13"
...
#
#   orphan_packages_in ТЕКСТ_ЖАЛОБ КАТАЛОГ_ТРЕБОВАНИЙ ПУТЬ_К_PIP
#
...
# и замолчать — значит оставить человека с жалобой, на которую нечего
# ответить.
#
...
# перечислены нигде, но живут спутником transformers, и советовать их снести
# — вредный совет.
#
...
_pip_conflicts_resolved() {
  ...
  debug "расхождения версий, замеченные по ходу, к концу установки устранились"
  return 0
}
...
check_dependency_health() {
  ...
  # Порядок строк здесь не косметика. Чек-лист показывает шесть заметок на
  # пункт, остальные остаются в журнале, — и в первый же раз на рабочем
  # сервере обрезало ровно ту строку, ради которой всё и писалось: команду
  # удаления. Поэтому сначала лекарство, потом жалобы, и объяснение — только
  # когда лекарства нет.
  local remedy
  ...
  # жалобы pip: те показывали состояние на середине пути. На экран идёт
  # образец, полный список — в журнал: он там нужен разбору постфактум, а на
  # экране вытесняет то, что человек может сделать прямо сейчас.
  local shown=0 limit=1
  [[ -z "${remedy}" ]] && limit=3
  while IFS= read -r line; do
    [[ -n "${line}" ]] || continue
    count=$((count + 1))
    if [[ ${shown} -lt ${limit} ]]; then
      ...
      debug "расхождение версий: ${line}"
    fi
  done <<< "${out}"
  [[ ${count} -gt ${shown} ]] \
    && hint "… и ещё $((count - shown)); полный список: ${pip} check"
  if [[ -z "${remedy}" ]]; then
    # Объяснение нужно ровно тогда, когда сделать нечего: это разные версии,
    # которых требуют сами движки. Печатать его рядом с командой удаления
    # значит поставить неверный диагноз — дело не в движках, а в пакете,
    # оставшемся от прежней версии.
...
diagnose_startup_failure() {
  ...
  # Сломанный YAML и отвергнутое значение — две разные беды с разным
  # лечением, а сообщение было одно на двоих и советовало проверить отступы
  # и кавычки. Обновление, сменившее тип параметра, отправляло человека
  # искать ошибку в файле, который в полном порядке. Теперь — дословная
  # строка из журнала: она называет параметр.
  local cfg_line
  cfg_line="$(set +o pipefail; grep -oE "Ошибка конфигурации: .*" <<<"${text}" | tail -1 || true)"
  if [[ -n "${cfg_line}" ]]; then
    error "Сервер отверг значение из файла конфигурации."
    printf '  %s\n' "${cfg_line}" >&2
    [[ -n "${python}" ]] && hint "Все несогласия с файлом: ${python} -m asrhub --check-config"
    hint "Запись прежней версии текущая принимает сама; если сообщение осталось, параметр придётся поправить руками."
    hint "Файл: ${data_dir:-КАТАЛОГ_ДАННЫХ}/config.yaml"
    return 0
  fi
  if grep -qiE "yaml|ScannerError|ParserError" <<<"${text}"; then
    error "Файл config.yaml не читается как YAML."
    hint "Отступы — только пробелами, значение с запятой — в кавычках."
    hint "Файл: ${data_dir:-КАТАЛОГ_ДАННЫХ}/config.yaml"
    return 0
  fi
  return 1
