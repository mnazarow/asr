...
class Настройки(dict):
    ...
    def __init__(self, каталог: Path, **поверх):
        ...
        self.paths = Пути()
        # Имя поля — как в настоящих настройках. Здесь стояло `config_path`,
        # и проверки не замечали, что `backup.py` читает то же неверное имя.
...
def test_расписание_раз_в_сутки_в_назначенное_время(стенд, monkeypatch):
    ...
    # Время замирает на середине дня. Без этого проверка падала ровно одну
    # минуту в сутки: между полуночью и 00:01 назначенный час ещё не
    # наступил, и «была вчера вечером» честно означает «рано». Поймано
    # ночным прогоном захода 35 — в 00:00:33.
    местное = time.localtime()
    полдень = time.mktime((местное.tm_year, местное.tm_mon, местное.tm_mday,
                           12, 0, 0, 0, 0, -1))
    monkeypatch.setattr(backup.time, "time", lambda: полдень)
    сейчас = полдень
    сегодня_в_00_01 = time.mktime((местное.tm_year, местное.tm_mon, местное.tm_mday,
                                   0, 1, 0, 0, 0, -1))
    assert сейчас > сегодня_в_00_01
