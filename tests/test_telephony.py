...
def test_выдержка_не_даёт_взять_недописанную_запись(tmp_path: Path):
    ...
    # Разговор кончился десять секунд назад: начался семьдесят секунд
    # назад и длился шестьдесят. Станция пишет строку журнала уже после
    # конца разговора, поэтому «конец в будущем» — это не тот случай.
...
def test_один_плохой_звонок_не_уносит_всю_порцию(tmp_path: Path):
    ...
    def падать_на_втором(звонок, **kwargs):
...
def test_два_захода_разом_не_читают_журнал_дважды(tmp_path: Path):
    ...
    def медленно(limit, **прочее):
        ...
        #
        # `**прочее` — чтобы подмена не рассыпалась от нового ключевого
        # аргумента источника (например «всё» при полном сборе): проверка
        # тут про замок, а не про подпись метода.
        time.sleep(0.35)
        итог = исходный(limit, **прочее)
        # Источник отдаёт пару «звонки, позиция после них»: позицию двигает
        # уже заход — за каждым разобранным звонком, а не всей пачкой разом.
        звонки = итог[0] if isinstance(итог, tuple) else итог
...
def test_источник_папка_берёт_каждый_файл_ровно_раз(tmp_path: Path):
    каталог = tmp_path / "monitor"
    wav(каталог / "in-79161234567-101-20260910.wav", секунд=30)
...
def test_длительность_записи_из_папки_берётся_из_файла(tmp_path: Path):
    """Ноль вместо длительности выключал разом и порог, и выдержку.

    Обе проверки написаны как «если длительность известна», а у звонка из
    каталога записей её неоткуда взять — ставился ноль. В итоге запись в
    четыре секунды уходила на распознавание при пороге «не брать короче
    двух минут», а файл, который станция ещё дописывала, брался сразу.
    """
    каталог = tmp_path / "monitor"
    wav(каталог / "in-79161234567-101-короткий.wav", секунд=3)
    ...
    assert "длинный" in очередь.вызовы[0]["filename"]
