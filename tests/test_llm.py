...
# ---------------------------------------------------------------------------
# Двадцать шестой заход: разбор свежего кода слоя
# ---------------------------------------------------------------------------


class _Ответы:
    """Клиент, отвечающий заранее заданным JSON по видам задач."""
...
def test_a_failed_retry_does_not_erase_the_answer_that_was_there(tmp_path):
    """«Разобрать заново» при лежащем сервере не должно стирать разбор.

    `llm_save` кладёт строку целиком, поэтому запись об ошибке затирала
    прежний ответ: одно нажатие кнопки — и резюме, причина, исход и
    действия пропадали, а на их месте оставалась строчка «сервер лёг».
    Записи без ответа отметка о сбое по-прежнему нужна — иначе разбор
    архива возвращается к ней бесконечно.
    """
    db = _архив(tmp_path, n=2)
    настройки = _Настройки()
    клиент = LLMClient(настройки, db)
    поток = LLMWorker(db, настройки, клиент)
    было = поток.analyze_job("l0")
    assert было["summary"] and было["outcome"]

    настройки.значения["llm_backend"] = "openai"
    клиент._openai = lambda *a, **k: (_ for _ in ()).throw(LLMError("сервер лёг"))
    with pytest.raises(LLMError):
        поток.analyze_job("l0", force=True)
    стало = db.llm_get("l0")
    assert стало["summary"] == было["summary"], "сбой затёр удачный разбор"
    assert стало["outcome"] == было["outcome"] and стало["actions"] == было["actions"]

    # Запись, у которой ответа не было, отметку о сбое получает.
    with pytest.raises(LLMError):
        поток.analyze_job("l1", force=True)
    assert db.llm_get("l1")["error"] == "сервер лёг"
    assert db.llm_pending(tasks.VERSION, limit=10) == []


def test_the_quotes_and_the_warnings_reach_the_stored_answer(tmp_path):
    """Цитата, по которой выбраны причина и исход, и замечания разбора
    доходят до карточки.

    Модель просят вернуть цитату — и это единственный способ проверить
    выбор из закрытого списка, не перечитывая разговор. Цитаты
    разбирались и выбрасывались: в таблице не было колонок. Замечание
    «модель ответила мимо списка» сохранялось только тогда, когда не
    вышло вообще ничего, то есть практически никогда.
    """
    db = _архив(tmp_path, n=1)
    настройки = _Настройки()
    клиент = _Ответы(main=json.dumps({
        "summary": "Клиент спрашивает о заказе.",
        "reason": "статус заказа или доставки",
        "reason_quote": "у меня заказ не пришёл",
        "outcome": "перезвоню послезавтра",           # мимо закрытого списка
        "outcome_quote": "перезвоню завтра",
        "actions": [], "resolved": False}, ensure_ascii=False))
    поток = LLMWorker(db, настройки, клиент)
    итог = поток.analyze_job("l0")
    assert итог["reason_quote"] == "у меня заказ не пришёл"
    assert итог["outcome_quote"] == "перезвоню завтра"
    assert итог["warnings"] == ["исход вне списка"]
    assert итог["summary"] and итог["error"] is None, "замечание не должно гасить разбор"
    # Свод за период считает такие ответы отдельно: это мера честности слоя.
    строки = db.llm_stats(tasks.VERSION, 0.0)["rows"]
    assert [р["warnings"] for р in строки] == [["исход вне списка"]]


def test_the_closed_list_takes_the_most_precise_item_not_the_first():
    """Из закрытого списка берётся самый точный пункт, а не первый попавшийся.

    Список пишут люди, и в нём соседствуют «оплата» и «оплата картой».
    Совпадение по вхождению возвращало первый подходящий пункт — то есть
    порядок строк в настройке решал, каким будет отчёт.
    """
    список = ["оплата", "оплата картой", "другое"]
    assert tasks._из_списка("оплата картой в приложении", список, ("друг",)) == (
        "оплата картой", True)
    # Ответ короче пункта — берётся самый узкий из содержащих его.
    assert tasks._из_списка("картой", ["оплата картой при доставке", "оплата картой"],
                            ("друг",)) == ("оплата картой", True)
    # Точное совпадение сильнее любого вхождения.
    assert tasks._из_списка("оплата", список, ("друг",)) == ("оплата", True)
    # Мимо списка — запасной пункт и честный признак «не попала».
    assert tasks._из_списка("выдумка", список, ("друг",)) == ("другое", False)
    assert tasks._из_списка("", список, ("друг",)) == ("другое", False)
    # Пустые строки в списке не должны совпадать со всем подряд.
    assert tasks._из_списка("что угодно", ["", "другое"], ("друг",)) == ("другое", False)


def test_the_client_follows_the_settings_it_was_given_later(tmp_path):
    """Ограничитель одновременности и проба доступности идут за настройками.

    Обе величины брались один раз: семафор — при сборке клиента, проба —
    на минуту без оглядки на адрес. Значит, правка `llm_max_concurrent` на
    странице настроек ничего не меняла до перезапуска, а после смены
    адреса сервера состояние ещё минуту описывало прежний сервер.
    """
    from asrhub.db import Database

    db = Database(tmp_path / "asrhub.db")
    настройки = _Настройки(llm_max_concurrent=1)
    клиент = LLMClient(настройки, db)
    assert клиент._семафор() is клиент._семафор()
    настройки.значения["llm_max_concurrent"] = 3
    новый = клиент._семафор()
    assert клиент._предел == 3 and новый is клиент._семафор()

    настройки.значения["llm_backend"] = "openai"
    спрошено = []
    клиент._probe_now = lambda: (спрошено.append(клиент.url), {"available": True})[1]
    клиент.probe()
    клиент.probe()
    assert спрошено == ["http://127.0.0.1:11434"], "проба должна кешироваться"
    настройки.значения["llm_url"] = "http://10.0.0.5:11434"
    клиент.probe()
    assert спрошено[-1] == "http://10.0.0.5:11434", "смена адреса не сбросила пробу"


def test_a_second_worker_thread_is_not_started_after_a_slow_stop(tmp_path):
    """Остановка, не дождавшаяся вызова модели, не должна плодить потоки.

    `stop()` отпускал ссылку на поток, даже когда тот ещё висел внутри
    вызова модели, и следующий `start()` поднимал второй такой же: два
    потока разбирали одну очередь и звали модель вдвоём.
    """
    db = _архив(tmp_path, n=1)
    поток = LLMWorker(db, _Настройки(), LLMClient(_Настройки(), db))
    ...
    поток._thread = занятый = _Занятый()
    поток.stop(timeout=0.01)
    assert поток._thread is занятый, "ссылка на живой поток потеряна"
    поток.start()
    assert поток._thread is занятый, "поднят второй поток разбора"
    assert not поток._stop.is_set(), "старый поток остановится и разбирать станет некому"


def test_a_record_that_just_failed_waits_before_the_next_attempt(tmp_path):
    """Запись с прежним ответом, на которой модель сорвалась, не крутится
    в разборе архива без перерыва."""
    from asrhub.llm.worker import ОСТЫТЬ
