...
def test_свои_поля_уходят_отдельно():
    """Свои поля Bitrix24 — отдельным запросом `crm.<сущность>.update`.

    До захода 42 они клались в комментарий ленты (`crm.timeline.comment.add`),
    где их молча не пишут; теперь — вторым запросом, правкой самой сделки.
    """
    данные = crm.собрать({"id": "j1"}, РАЗБОР_ДЛЯ_CRM)
    настройки = _crm(crm_fields="score = UF_CRM_BALL\nsentiment=UF_CRM_TON")
    _, _, тело = crm.запрос(данные, настройки, entity_id="42")
    assert "UF_CRM_BALL" not in json.loads(тело)["fields"]
    правка = crm.запросы(данные, настройки, entity_id="42")[1]
    assert правка.url.endswith("/crm.deal.update.json")
    поля = json.loads(правка.body)["fields"]
    assert json.loads(правка.body)["id"] == "42"
...
def test_возражение_без_отработки_подсказывается():
    """С захода 42 — только когда окно в три реплики прошло без отработки."""
    помощник = Помощник.из_настроек({}, agent="оператор")
    ...
    for номер, реплика in enumerate(("понятно", "так", "ну вот")):
        подсказки += помощник.добавить(реплика, speaker="оператор",
                                       start=6 + номер, end=7 + номер)
