...
def test_очистка_отвечает_словами_а_не_сырым_json(страница):
    """Заход 44: «Очистка выполнена» показывала `{"jobs": 0, "metrics": 0, …}`.

    Теперь — сколько убрано и сжата ли база, а если нет — почему.
    """
    _открыть(страница, "system")
    страница.wait_for_selector("#btn-cleanup", timeout=15000)
    страница.click("#btn-cleanup")
    своя = '#toasts .toast:has-text("Очистка выполнена") .t-hint'
    страница.wait_for_selector(своя, timeout=15000)
    подсказка = страница.inner_text(своя)
