...
def test_безнадёжные_отложенные_не_забивают_очередь(tmp_path):
    ...
    Срок считается от времени ОТКЛАДЫВАНИЯ (`imported_at`), а не от времени
    разговора: заход тридцать пятый (см. `test_review_35_data.py`) показал,
    что по времени разговора из очереди мгновенно выпадал любой архивный
    звонок. Поэтому старьём здесь состарены обе отметки, а не только
    `started_at`: у по-настоящему безнадёжного звонка дата откладывания
    такая же старая — его просто никто не трогал третьи сутки.
    """
