...
def test_update_replaces_files_without_overwriting_them_in_place(repo_root: Path,
                                                                  tmp_path: Path):
    """Обновление заменяет тот самый каталог, из которого запущено.

    Это безопасно ровно до тех пор, пока замена идёт через `rm` и `mv`:
    оболочка держит открытым прежний файл по индексному дескриптору и
    спокойно дочитывает его до конца, даже если имя уже занято новой
    версией. Стоит заменить это на запись поверх того же файла — и bash
    дочитает с прежнего смещения уже новое содержимое, выполнив обрывок
    случайной строки. Ошибка выглядит как «syntax error» на середине
    обновления, и по журналу её не восстановить.

    Проверяем и порядок в скрипте, и само поведение — на макете, заметно
    более длинном, чем блок чтения оболочки.
    """
    source = (repo_root / "scripts" / "update.sh").read_text(encoding="utf-8")
    замена = re.search(r'for item in .*?\n\s*rm -rf "\$\{PREFIX:\?\}/\$\{item\}"\n'
                       r'\s*mv "\$\{staged\}" "\$\{PREFIX:\?\}/\$\{item\}"', source, re.S)
    assert замена, "файлы заменяются не через rm + mv — под вопросом сам запущенный скрипт"

    чужой = tmp_path / "новая-версия.sh"
    чужой.write_text("printf 'ЧУЖОЙ КОД\\n'\n" * 6000, encoding="utf-8")
    наполнение = ("# наполнение " + "х" * 60 + "\n") * 900

    безопасно = tmp_path / "как-в-обновлении.sh"
    безопасно.write_text('printf "начало\\n"\n'
                         f'rm -f "$0" && cp "{чужой}" "$0"\n'
                         + наполнение + 'printf "конец\\n"\n', encoding="utf-8")
    опасно = tmp_path / "поверх-того-же-файла.sh"
    опасно.write_text('printf "начало\\n"\n'
                      f'cat "{чужой}" > "$0"\n'
                      + наполнение + 'printf "конец\\n"\n', encoding="utf-8")
    assert безопасно.stat().st_size > 64 * 1024, "макет короче блока чтения"

    хорошо = run_bash(f'bash "{безопасно}"')
    assert "конец" in хорошо.stdout, f"замена через rm+mv сломала сценарий: {хорошо.stderr[:300]}"
    assert "ЧУЖОЙ КОД" not in хорошо.stdout + хорошо.stderr

    # А так — ломается; ради этого различия порядок и закреплён.
