...
ПО_УМОЛЧАНИЮ: list[dict[str, Any]] = [
    ...
    {"id": "next_step", "label": "Назвал следующий шаг",
     ...
     # («переда»), их оставляем.
     #
     # «Вышлю» и «вышлем» — в кавычках, то есть по точной форме. Основа у
     # них «вышл», и ровно её даёт «вышло»: пункт засчитывался оператору
     # за «так вышло» и «вышло недоразумение» — за слова, которые
     # следующего шага не называют вовсе. Развести их по основам нельзя
     # никаким списком, поэтому здесь перечислены формы будущего времени.
     "any": ["перезвоню", "отправлю", '"вышлю"', '"вышлем"', '"вышлет"',
             "передадим", "передать", "сделаем", "свяжемся", "решим",
             "уточню", "оформлю"],
     "where": "any"},
