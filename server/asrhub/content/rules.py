...
def parse_any(варианты: list[str]) -> Node:
    ...
    for вариант in варианты or []:
        строка = str(вариант).strip()
        # Примета в кавычках — как в правиле: совпадение по ТОЧНОЙ форме, а
        # не по основе. Без этого некоторые приметы записать было нечем.
        # Пример из скрипта по умолчанию: «вышлю» даёт основу «вышл», и её
        # же даёт «вышло» — пункт «Назвал следующий шаг» засчитывался
        # оператору за «так вышло». По основам эти слова не развести
        # никакими списками; кавычки разводят.
        точно = len(строка) >= 2 and строка[0] == '"' and строка[-1] == '"'
        if точно:
            формы = tuple(words(строка[1:-1]))
            if формы:
                фразы.append(Phrase(формы, True, строка[1:-1].strip()))
            continue
        основы = tuple(stem(w) for w in words(строка))
        if основы:
            фразы.append(Phrase(основы, False, строка))
    if not фразы:
