...
class PhoneRequest:
    ...
    def path_key(self) -> str:
        """Тот же ключ, но пригодный для имени файла и каталога.

        `call_id` приходит от клиента и в исходном виде попадал прямо в путь:
        значение вида «../../../имя» уводило запись скачанного файла за
        пределы каталога загрузок, а расширение задавал адрес источника —
        то есть маршрут позволял положить свой .js или .html куда угодно,
        куда пишет служба, и рекурсивно удалить чужой каталог при уборке.

        Здесь остаются только буквы, цифры и три безопасных знака; всё
        прочее — точки, слэши, обратные слэши, управляющие символы —
        заменяется подчёркиванием. Сам `call_id` не трогается: он уходит в
        `group_id` и в обратный вызов как есть, и заказчик обязан получить
        обратно ровно то, что прислал.
        """
        return safe_path_key(self.uuid)
...
#: Сколько знаков ключа оставлять в имени файла. Длиннее незачем: имя и так
#: несёт имя исходного файла, а у файловых систем есть предел на длину.
_PATH_KEY_MAX = 80


def safe_path_key(value: str) -> str:
    """Превращает пришедший снаружи идентификатор в безопасное имя.

    Отдельная функция, потому что тот же ключ становится и именем каталога,
    и частью имени файла: два места, и оба обязаны получить одно и то же.
    """
    очищено = re.sub(r"[^0-9A-Za-zА-Яа-яЁё._-]", "_", value or "")
    # Точки убираем целиком: «..» — это переход на уровень выше, а точка в
    # начале делает файл скрытым.
    очищено = очищено.replace(".", "_").strip("_")
    return (очищено[:_PATH_KEY_MAX] or "без-имени")
