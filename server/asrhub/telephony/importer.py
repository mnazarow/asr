...
class Импортёр:
    ...
    def _scan(self, *, limit: int = ПОРЦИЯ, всё: bool = False,
        ...
        if источник == "ami":
            ...
            новые = self._из_csv(limit, всё=всё)
    ...
    def _из_csv(self, limit: int, *, всё: bool = False) -> list[Звонок]:
        путь = self.станция.путь("cdr_file")
        if путь is None:
            raise ConfigError(
                f"«{self.станция.name}»: не задан журнал звонков.",
                hint="Обычно это /var/log/asterisk/cdr-csv/Master.csv")
        # Позиция чтения одна и та же и для обычного захода, и для сбора
        # архива. Раньше при `всё=True` она обнулялась на КАЖДОМ заходе, а
        # сбор состоит из заходов подряд — и каждый перечитывал один и тот
        # же первый кусок файла. На архиве в сорок тысяч звонков из него
        # поднимались первые двести, после чего сбор упирался в предел и
        # рапортовал «остановлено», не прочитав девяносто девять процентов
        # журнала. Обнуление делается один раз, в начале сбора.
