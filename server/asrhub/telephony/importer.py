...
class Импортёр:
    ...
    def _scan(self, *, limit: int = ПОРЦИЯ, всё: bool = False,
        ...
        if источник == "ami":
            ...
            новые, позиция_после = self._из_csv(limit, всё=всё, ключ=ключ)
            ...
            if окно is not None and звонок.started_at:
                ...
                if not (окно[0] <= звонок.started_at <= окно[1]):
                    ...
                    if звонок.конец_строки >= 0 and позиция_после is not None:
                        self.db.set_kv(ключ, звонок.конец_строки)
                    continue
            try:
                почему = self._взять(
        ...
        if позиция_после is not None:
            ...
            self.db.set_kv(ключ, позиция_после)
        with self._lock:
    ...
    def собрать(self, *, режим: str = "all", since: float = 0.0,
        ...
        if self.станция.source == "cdr_csv":
            self.db.set_kv(self._ключ_позиции_сбора if режим == "period"
                           else self._ключ_позиции, 0)
        self.сбор = {
    ...
    def забыть_позицию(self) -> dict[str, Any]:
        ...
        self.db.set_kv(self._ключ_позиции, 0)
    ...
    def _из_csv(self, limit: int, *, всё: bool = False,
                ключ: str = "") -> tuple[list[Звонок], int]:
        ...
        позиция = int(self.db.get_kv(ключ or self._ключ_позиции, 0) or 0)
        # Позиция НЕ сдвигается здесь: её двигает `_scan` за каждым
        # обработанным звонком (см. там).
        return читать_csv(путь, offset=позиция, limit=limit)
