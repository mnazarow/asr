...
def найти_запись(звонок: Звонок, каталог: Path, *, шаблон: str = "",
                 окно_дней: int = 2, окно_минут: int = 120) -> Path | None:
    ...
    for путь in каталог.rglob("*"):
        ...
        # Окно — вокруг начала разговора, в обе стороны. Без него сюда
        # попадал файл, записанный через несколько суток после звонка:
        # `граница` выше отсекает только слишком СТАРЫЕ файлы, а вперёд
        # ограничения не было вовсе, и разговор той же пары номеров,
        # состоявшийся в четверг, доставался звонку понедельника — просто
        # потому, что более близкой записи в каталоге не нашлось. В архив
        # ложилась чужая расшифровка под правильными номерами, и заметить
        # это можно было, только прослушав запись. Агент на станции ровно
        # здесь окно имеет (`match_window`), сервер — теперь тоже.
