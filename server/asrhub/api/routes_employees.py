...
def завести(request: Request, тело: dict[str, Any] = Body(...),
    ...
    if not поля.get("external_id"):
        ...
        ключ = номер or фио
        прежняя = db.employee_by_key(str(поля["source"]), ключ)
        # Совпавший ключ — не всегда тот же человек. Ключ по ФИО у полных
        # тёзок один: второй «Иванов Пётр» без номера молча сливался с
        # первым — карточка первого переписывалась полями второго, и в
        # справочнике оставался один человек из двух. Ключ по номеру
        # совпадает у соседей по общему телефону. Тот же человек — это
        # тот же номер И то же ФИО: такую карточку правим, как раньше;
        # иначе заводим новую с ключом, которого ещё нет, и говорим об этом.
        прежнее_фио = "|".join(str((прежняя or {}).get(п) or "") for п in
                               ("last_name", "first_name", "middle_name")).lower()
        if прежняя is not None and not (номер and фио == прежнее_фио):
            основа = фио if номер else ключ
            ключ = основа
            счёт = 2
            while db.employee_by_key(str(поля["source"]), ключ) is not None:
                ключ = f"{основа}#{счёт}"
                счёт += 1
            замечание = (
                f"Карточка «{_имя(прежняя)}» с тем же "
                f"{'внутренним номером' if номер else 'ФИО'} уже есть — заведена "
                "вторая. Если это один человек, удалите лишнюю.")
        поля["external_id"] = ключ
    try:
        карточка = db.employee_save(поля)
    except ASRHubError as exc:
        raise error_response(exc) from exc
    db.add_event(None, "employee_added", f"Заведён сотрудник: {_имя(карточка)}")
    return {**карточка, **({"warning": замечание} if замечание else {})}
