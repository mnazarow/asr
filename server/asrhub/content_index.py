...
class ContentIndex:
    ...
    def analyze_job(self, job_id: str, *, job: dict[str, Any] | None = None,
        ...
        if save:
            ...
            # Обратной записи в CRM здесь больше нет. Этот метод зовут и
            # пересчёт («Пересчитать», «Пересчитать всё»), и открытие
            # карточки, и разбор архива — и каждый из них слал в карточку
            # сделки новое примечание. Теперь примечание уходит один раз, по
            # свежей записи: из `on_job_completed` (и у переписки — из её
            # маршрута), а если по записи будет смысловой разбор — после него.
    ...
    def в_crm(self, job_id: str, разбор: dict[str, Any] | None) -> None:
        """Примечание по свежей записи — сейчас или после смыслового разбора.

        Ошибки внутри не поднимаются: чужая система, которая не отвечает, не
        должна ронять разбор записи.
        """
        try:
            crm.после_разбора(self.db, self.settings, job_id, разбор)
        except Exception as exc:                             # noqa: BLE001
