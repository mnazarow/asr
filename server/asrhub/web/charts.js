...
function fmtNum(value, digits) {
  ...
  function hbars(host, config) {
    ...
    // Ось делится пополам, только когда в наборе есть значения обоих
    // знаков. Раньше хватало одного отрицательного, и набор, где все
    // значения отрицательные (индекс NPS по сотрудникам, отклонение от
    // нормы вниз), рисовался в левой половине: половина ширины пустовала,
    // а подписи значений уходили под имена строк и наползали на них.
    const знаки = items.map((i) => Math.sign(i.value || 0));
    const двусторонние = config.diverging === true
      || (знаки.some((з) => з > 0) && знаки.some((з) => з < 0));
    ...
    const двусторонний = двусторонние;
    // Односторонний набор из одних отрицательных рисуется по модулю: длина
    // столбца — это величина, а знак несут цвет и подпись. Иначе «−85»
    // оказывался бы короче «−9», то есть худший выглядел бы лучшим.
    const по_модулю = !двусторонний && знаки.some((з) => з < 0);
    const предел = Math.max(...значения.map(Math.abs), config.absMin || 0) || 1;
    const max = по_модулю ? предел : (Math.max(...значения, 0) || 1);
    const s = status();
    ...
    items.forEach((item, index) => {
      ...
      const длина = двусторонний
        ...
        : Math.max(2, (Math.abs(по_модулю ? значение : Math.max(0, значение)) / max) * ctx.iw);
      ...
      const цвет = item.color || (двусторонний
        ...
        : по_модулю ? s.err : colors[index % colors.length]);
      const rect = el('rect', {
      ...
      // Подпись растёт в ту же сторону, что и столбец. Но если с этой
      // стороны места нет — а у короткого отрицательного столбца его нет
      // никогда, — она уходит внутрь столбца: там она читается, а поверх
      // имени строки не читается ни она, ни имя.
      const влево = значение < 0 && двусторонний;
      const запас = подпись.length * 6.5 + 10;
      const тесно = влево && (x - ctx.pad.left) < запас;
      el('text', {
        x: тесно ? x + 6 : влево ? x - 8 : x + длина + 8,
        y: y + rowHeight / 2 + 4,
        'text-anchor': влево && !тесно ? 'end' : 'start',
        fill: ink(), 'font-size': 11.5,
      }, ctx.svg).textContent = подпись;
    });
    return ctx.svg;
  }
