...
def ks_test(первая: list[float], вторая: list[float]) -> dict[str, Any]:
    ...
    i = j = 0
    d = 0.0
    # Расхождение меряется в точках, а не после каждого шага: при
    # совпадающих значениях функция распределения прыгает у обеих выборок
    # разом, и мерить между этими прыжками нельзя. Прежний цикл двигал по
    # одному указателю и успевал «увидеть» разницу в целый блок повторов:
    # две одинаковые выборки по шестьсот значений, где двести — один и тот
    # же уровень уверенности (а так и бывает: запись из кеша копирует
    # уверенность оригинала), давали D = 0,29 вместо 0,05 — и вердикт
    # «критично» на ровном месте.
    #
    # Поэтому за шаг проходим ВЕСЬ блок одинаковых значений в обеих
    # выборках и только потом сравниваем доли.
    while i < n and j < m:
        значение = a[i] if a[i] <= b[j] else b[j]
        while i < n and a[i] == значение:
            i += 1
        while j < m and b[j] == значение:
            j += 1
        d = max(d, abs(i / n - j / m))
