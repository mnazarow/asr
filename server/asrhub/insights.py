...
class Insights:
            ...
            for метка in метки:
                ...
                for имя, _ in Database.CONTENT_METRICS:
                    ...
                    if значение is None:
                        continue
                    if имя in средние:
                        цель[имя] = цель.get(имя, 0.0) + float(значение) * n
                        цель[f"_{имя}_вес"] = цель.get(f"_{имя}_вес", 0.0) + n
                    elif имя == "money_max":
    ...
    def findings(self, period: str = "week", owner: str | list[str] | None = None,
        ...
        # 8. Разбор ещё не закончен — об этом надо сказать до выводов, а не
