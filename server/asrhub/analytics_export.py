...
#: С каких знаков начинается формула для Excel и LibreOffice. Табуляция и
#: перевод каретки тоже: ими обходят проверку на первый знак.
_ОПАСНОЕ_НАЧАЛО = ("=", "+", "-", "@", "\t", "\r")


def _опасно(значение: Any) -> bool:
    """Строка, которую Excel или LibreOffice примут за формулу."""
    return isinstance(значение, str) and значение[:1] in _ОПАСНОЕ_НАЧАЛО


def _безопасно(значение: Any) -> Any:
    """Клетка CSV, которая не станет формулой при открытии файла.

    Имя файла приходит от того, кто загрузил запись, метка — свободное поле
    формы, и то и другое попадает в клетку как есть. Excel и LibreOffice
    превращают строку, начинающуюся с «=», в живую формулу: `=HYPERLINK(…&A1)`
    утягивает содержимое соседней клетки на чужой адрес, а этот файл по
    замыслу уходит руководителю и в бухгалтерию.

    В CSV другого способа сказать «это текст» нет, кроме апострофа впереди,
    и Excel его покажет: «'=HYPERLINK…» — цена того, что ссылка не сработает.
    В книге Excel апостроф не нужен и не пишется: там клетка просто
    помечается текстовой (см. `_собрать_xlsx`). Числа и даты не трогаются:
    формулой становится только строка.
    """
    if _опасно(значение):
        return "'" + значение
    return значение
...
def _клетка_csv(значение: Any) -> Any:
    """Клетка CSV для Excel с русскими настройками.

    Дробь — через запятую: «12.5» русский Excel читает как дату «12.май», а
    «0.35» — как текст, и ни то, ни другое уже не сложить. Разделитель
    колонок в том же файле — точка с запятой, ровно под эти настройки.
    Нечисло («nan», бесконечность) — пустая клетка, а не слово.
    """
    if isinstance(значение, float):
        if значение != значение or значение in (float("inf"), float("-inf")):
            return ""
        return repr(значение).replace(".", ",")
    return _безопасно(значение)
...
def _собрать_csv(таблицы: list[Таблица], period: str) -> bytes:
    ...
    with zipfile.ZipFile(буфер, "w", zipfile.ZIP_DEFLATED) as архив:
        for имя, заголовки, строки in таблицы:
            ...
            писарь.writerow([_безопасно(з) for з in заголовки])
            писарь.writerows([[_клетка_csv(з) for з in строка] for строка in строки])
...
def _как_текст(клетка: Any) -> None:
    """Строка, похожая на формулу, — в книге текстом, без апострофа.

    openpyxl считает формулой всё, что начинается с «=», и пишет это как
    формулу. Прежний апостроф впереди попадал внутрь текста клетки, и Excel
    его показывал: «'=HYPERLINK…», «'-5 минут». Текстовый тип клетки и
    признак «текст с апострофом» в стиле (quotePrefix) делают то же без
    лишнего знака: ни при открытии, ни при правке клетка формулой не станет.
    """
    if _опасно(клетка.value):
        клетка.data_type = "s"
        клетка.quotePrefix = True
...
def _собрать_xlsx(таблицы: list[Таблица], title: str) -> bytes:
    ...
    for имя, заголовки, строки in таблицы:
        ...
        for строка in строки:
            ...
            for клетка in лист[лист.max_row]:
                _как_текст(клетка)
