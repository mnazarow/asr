...
def _безопасно(значение: Any) -> Any:
    """Клетка, которая не станет формулой при открытии книги.
    ...
    Апостроф впереди — принятый способ сказать «это текст»: сам он в клетке
    не показывается. Числа и даты не трогаются: формулой становится только
    строка.
    """
    if isinstance(значение, str) and значение[:1] in _ОПАСНОЕ_НАЧАЛО:
        return "'" + значение
    return значение
...
def _собрать_csv(таблицы: list[Таблица], period: str) -> bytes:
    ...
    with zipfile.ZipFile(буфер, "w", zipfile.ZIP_DEFLATED) as архив:
        for имя, заголовки, строки in таблицы:
            ...
            писарь.writerows([[_безопасно(з) for з in строка] for строка in строки])
...
def _собрать_xlsx(таблицы: list[Таблица], title: str) -> bytes:
    ...
    for имя, заголовки, строки in таблицы:
        ...
        лист.append([_безопасно(з) for з in заголовки])
        for клетка in лист[1]:
            клетка.font = шапка
            клетка.alignment = Alignment(vertical="center")
        for строка in строки:
            лист.append(["" if v is None else _безопасно(v) for v in строка])
