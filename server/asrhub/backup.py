...
def восстановить(db: Any, settings: Any, name: str, *, what: str = "settings",
    ...
    if итог.get("database"):
        база = Path(getattr(settings.paths, "db", "") or Path(settings.paths.data) / "asrhub.db")
...
def _вернуть_базу(файл: Path, settings: Any) -> dict[str, Any]:
    ...
    цель = Path(getattr(settings.paths, "db", "") or Path(settings.paths.data) / "asrhub.db")
...
def применить_отложенное(путь_базы: Path | str) -> str:
    ...
    if цель.exists():
        try:
            соединение = sqlite3.connect(str(цель))
            try:
                соединение.execute("PRAGMA wal_checkpoint(TRUNCATE)")
            finally:
                соединение.close()
        except sqlite3.Error as exc:
            log.warning("Журнал прежней базы не влит перед восстановлением: %s", exc)
        цель.replace(запасная)
        for хвост in ("-wal", "-shm"):
            спутник = Path(str(цель) + хвост)
            if спутник.exists():
                # Под именем, по которому SQLite найдёт его у отложенной базы.
...
def принять(settings: Any, имя_файла: str, поток: Any) -> dict[str, Any]:
    ...
    try:
        ...
        черновик.touch(mode=0o600, exist_ok=True)
        os.chmod(черновик, 0o600)
