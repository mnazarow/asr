...
_m(MetricSpec(
    ...
    label="Расхождение моделей", labels=("model",),
    description=(
        "Среднее расхождение (WER одной расшифровки относительно другой) "
        "между основной моделью и контрольной по прогонам за неделю; метка "
        "model — основная модель. Точность без эталона это не меряет: "
