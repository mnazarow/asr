...
СВОД: list[tuple[str, str]] = [
    ...
    ("Пауз на запись", "pauses"),
    ("Длинных пауз на запись (отраслевой порог)", "long_pauses"),
    ...
    # Порог подставляется из отчёта: это настройка `content_dead_air_s`, и
    # «от 3 с» в заголовке сервера, где её подняли до пяти, было неправдой.
    ("Заметная тишина (паузы от {тишина} с), с", "dead_air_s"),
...
def _свод(отчёт: dict[str, Any]) -> list[list[Any]]:
    ...
    for подпись, ключ in СВОД:
        ...
        строки.append([_с_порогом(подпись, отчёт), стало, было, изменение])
    return строки


def _с_порогом(подпись: str, отчёт: dict[str, Any]) -> str:
    """Подпись с порогами, по которым отчёт посчитан на этом сервере."""
    if "{тишина}" not in подпись:
        return подпись
    порог = float((отчёт.get("thresholds") or {}).get("dead_air_s") or 3.0)
    return подпись.replace("{тишина}", f"{порог:g}".replace(".", ","))
...
def _нормы(отчёт: dict[str, Any]) -> list[list[Any]]:
    статусы = {"inside": "в норме", "above": "выше нормы", "below": "ниже нормы",
               "outlier_high": "выброс: выше", "outlier_low": "выброс: ниже"}
    return [[н.get("title"), н.get("unit"), н.get("current"), н.get("median"),
             н.get("q1"), н.get("q3"), н.get("low"), н.get("high"), н.get("n"),
             статусы.get(н.get("status") or "", "мало данных")]
            for н in (отчёт.get("norms") or {}).get("items") or []]
...
ЛИСТЫ: list[tuple[str, list[str], Any]] = [
    ...
    ("Нормы", ["Показатель", "Единица", "Медиана периода", "Медиана нормы", "Q1", "Q3",
               "Нижняя граница", "Верхняя граница", "Записей в базе", "Относительно нормы"],
     _нормы),
...
def _таблицы(отчёт: dict[str, Any]) -> list[tuple[str, list[str], list[list[Any]]]]:
    ...
    for имя, заголовки, достать in ЛИСТЫ:
        try:
            ...
            continue
        if строки:
            out.append((имя, [_с_порогом(з, отчёт) for з in заголовки], строки))
    return out
